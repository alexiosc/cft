...
module constant_store (nruen, raddr, ibus);
   ...
   // The constant store decodes RADDR 110aa, with aa being the least
   // significant bits of the constant to output.

   wire [7:0] 	 y;
   demux_138 cs_demux (.g1(raddr[4]), .g2a(raddr[2]), .g2b(nruen), .a({2'b00, raddr[3]}), .y(y));
   assign ncsoe = y[1];

   buffer_541 cs_lo (.a({6'b000000, raddr[1:0]}), .y(ibus[7:0]),  .oe1(ncsoe), .oe2(1'b0));
   buffer_541 cs_hi (.a(8'h00),                   .y(ibus[15:8]), .oe1(ncsoe), .oe2(1'b0));
