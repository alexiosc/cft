...
handle:		.data @			; TBD, but this will do for now.
cts:		JMP drv.cts		; Clear to send? (always yes for the DFP)
send:		JMP drv.send
dsr:		JMP drv.dsr
read:		JMP drv.read		; Blocking read
status:		JMP os.retnop		; *** NOT AVAILABLE
ctl:		JMP os.retnop		; *** NOT AVAILABLE
...
cts:		errno(err.NOERROR)
		JMP rtt1		; The DFP is always ready to receive

;;; Send out a character.

send:		errno(err.NOERROR)
		LOAD ARG0
...
dsr:		LMOV(p0.ERRNO, 0)
		...
		STORE p0.DFP_LASTC
		SNA
		JMP os.ret1		; 1 = yes, there are characters
		JMP os.ret0		; 0 = no, no characters
...
read:		LMOV(p0.ERRNO, 0)
		...
		AND p0.BYTELO		; Keep the low 8 bits
