EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 9 12
Title "Microcode Sequencer"
Date ""
Rev "2020"
Comp ""
Comment1 "CTL"
Comment2 ""
Comment3 ""
Comment4 "sequencer.v"
$EndDescr
Text HLabel 1150 2300 0    50   Input ~ 0
~ENDEXT
Text HLabel 1150 2100 0    50   Input ~ 0
~HALT
Text HLabel 1150 2000 0    50   Input ~ 0
CLK4
Text Label 5000 1900 2    50   ~ 0
UPC0
Text Label 5000 2000 2    50   ~ 0
UPC1
Text Label 5000 2100 2    50   ~ 0
UPC2
Text Label 5000 2200 2    50   ~ 0
UPC3
Text Label 10450 4300 2    50   ~ 0
COND[0..4]
Text Label 10450 3600 2    50   ~ 0
WADDR[0..4]
Text HLabel 10500 2900 2    50   Output ~ 0
RADDR[0..4]
Text HLabel 10500 3600 2    50   Output ~ 0
WADDR[0..4]
Text HLabel 10500 4300 2    50   Output ~ 0
COND[0..4]
Text Label 10450 5000 2    50   ~ 0
ACTION[0..3]
Text HLabel 10500 5000 2    50   Output ~ 0
ACTION[0..3]
Text Notes 6950 6700 0    50   ~ 0
2019 Design Update: both micro-Address Vector and micro-Control Vector have changed significantly\nin this version. Fields are much more vertical now, but the processor is also much more capable.
Text Notes 6950 6400 0    63   ~ 13
2019 Design Update
Text HLabel 10500 5850 2    50   Output ~ 0
~MEM
Text HLabel 10500 5950 2    50   Output ~ 0
~IO
Text HLabel 10500 6050 2    50   Output ~ 0
~R
Text HLabel 10500 6150 2    50   Output ~ 0
~WEN
Text HLabel 10500 6250 2    50   Output ~ 0
~END
Text Notes 9200 5650 1    100  ~ 20
Microcode Control Vector
Wire Wire Line
	9300 3550 9700 3550
Wire Wire Line
	9300 3450 9700 3450
Wire Wire Line
	9300 3350 9700 3350
Wire Wire Line
	9300 3250 9700 3250
Wire Wire Line
	9300 3150 9700 3150
Wire Wire Line
	9300 3850 9700 3850
Wire Wire Line
	9300 3950 9700 3950
Wire Wire Line
	9300 4050 9700 4050
Wire Wire Line
	9300 4150 9700 4150
Wire Wire Line
	9300 4250 9700 4250
Wire Wire Line
	9300 4550 9700 4550
Wire Wire Line
	9300 4650 9700 4650
Wire Wire Line
	9300 4750 9700 4750
Wire Wire Line
	9300 4850 9700 4850
Wire Wire Line
	9300 4950 9700 4950
Wire Wire Line
	9300 5250 9700 5250
Wire Wire Line
	9300 5350 9700 5350
Wire Wire Line
	9300 5450 9700 5450
Wire Wire Line
	9300 5550 9700 5550
Wire Wire Line
	9300 5850 10500 5850
Wire Wire Line
	9300 5950 10500 5950
Wire Wire Line
	9300 6050 10500 6050
Wire Wire Line
	9300 6250 10500 6250
Wire Wire Line
	9300 6150 10500 6150
Text Label 9300 5550 0    50   ~ 0
ACTION3
Text Label 9300 5450 0    50   ~ 0
ACTION2
Text Label 9300 5350 0    50   ~ 0
ACTION1
Text Label 9300 5250 0    50   ~ 0
ACTION0
Text Label 9300 4950 0    50   ~ 0
COND4
Text Label 9300 4850 0    50   ~ 0
COND3
Text Label 9300 4750 0    50   ~ 0
COND2
Text Label 9300 4650 0    50   ~ 0
COND1
Text Label 9300 4550 0    50   ~ 0
COND0
Text Label 9300 4250 0    50   ~ 0
WADDR4
Text Label 9300 4150 0    50   ~ 0
WADDR3
Text Label 9300 4050 0    50   ~ 0
WADDR2
Text Label 9300 3950 0    50   ~ 0
WADDR1
Text Label 9300 3850 0    50   ~ 0
WADDR0
Text Label 9300 3550 0    50   ~ 0
RADDR4
Text Label 9300 3450 0    50   ~ 0
RADDR3
Text Label 9300 3350 0    50   ~ 0
RADDR2
Text Label 9300 3250 0    50   ~ 0
RADDR1
Text Label 9300 3150 0    50   ~ 0
RADDR0
Text Label 9300 6250 0    50   ~ 0
~END
Text Label 9300 6150 0    50   ~ 0
~WEN
Text Label 9300 6050 0    50   ~ 0
~R
Text Label 9300 5950 0    50   ~ 0
~IO
Text Label 9300 5850 0    50   ~ 0
~MEM
Entry Wire Line
	9800 3050 9700 3150
Entry Wire Line
	9800 3150 9700 3250
Entry Wire Line
	9800 3250 9700 3350
Entry Wire Line
	9800 3350 9700 3450
Entry Wire Line
	9800 3450 9700 3550
Entry Wire Line
	9800 3750 9700 3850
Entry Wire Line
	9800 3850 9700 3950
Entry Wire Line
	9800 3950 9700 4050
Entry Wire Line
	9800 4050 9700 4150
Entry Wire Line
	9800 4150 9700 4250
Entry Wire Line
	9800 4450 9700 4550
Entry Wire Line
	9800 4550 9700 4650
Entry Wire Line
	9800 4650 9700 4750
Entry Wire Line
	9800 4750 9700 4850
Entry Wire Line
	9800 4850 9700 4950
Entry Wire Line
	9800 5150 9700 5250
Entry Wire Line
	9800 5250 9700 5350
Entry Wire Line
	9800 5350 9700 5450
Entry Wire Line
	9800 5450 9700 5550
Entry Bus Bus
	9800 3000 9900 2900
Entry Bus Bus
	9800 3700 9900 3600
Entry Bus Bus
	9800 4400 9900 4300
Entry Bus Bus
	9800 5100 9900 5000
Wire Bus Line
	9900 3600 10500 3600
Wire Bus Line
	9900 4300 10500 4300
Wire Bus Line
	9900 5000 10500 5000
Text Label 10450 2900 2    50   ~ 0
RADDR[0..4]
Wire Bus Line
	9900 2900 10500 2900
Text HLabel 1150 5300 0    50   Input ~ 0
~IRQSµC
Text HLabel 1150 5400 0    50   Input ~ 0
~RSTHOLD
Text HLabel 1150 4200 0    50   Input ~ 0
~COND
Text HLabel 6650 6100 2    50   3State ~ 0
FPD[0..7]
$Comp
L power:+5V #PWR0108
U 1 1 5E28DA06
P 7650 5300
AR Path="/5DEAC282/5E28DA06" Ref="#PWR0108"  Part="1" 
AR Path="/5DEAC282/5DEAA264/5E28DA06" Ref="#PWR?"  Part="1" 
F 0 "#PWR0108" H 7650 5150 50  0001 C CNN
F 1 "+5V" H 7665 5473 50  0000 C CNN
F 2 "" H 7650 5300 50  0001 C CNN
F 3 "" H 7650 5300 50  0001 C CNN
	1    7650 5300
	1    0    0    -1  
$EndComp
Text Label 7000 3750 0    50   ~ 0
COND0
Text Label 7000 3850 0    50   ~ 0
COND1
Text Label 7000 3950 0    50   ~ 0
COND2
Text Label 7000 4050 0    50   ~ 0
COND3
Text Label 7000 4150 0    50   ~ 0
COND4
Wire Wire Line
	7350 3750 7000 3750
Wire Wire Line
	7350 3850 7000 3850
Wire Wire Line
	7350 3950 7000 3950
Wire Wire Line
	7350 4050 7000 4050
Wire Wire Line
	7350 4150 7000 4150
Wire Wire Line
	7350 4250 7000 4250
Text Label 7000 4250 0    50   ~ 0
~END
Text Label 6650 5500 2    50   ~ 0
~MEM
Text Label 6650 5600 2    50   ~ 0
~IO
Text Label 6650 5700 2    50   ~ 0
~R
Text Label 6650 5800 2    50   ~ 0
~WEN
Text Label 6650 5900 2    50   ~ 0
~END
Text Label 6650 3600 2    50   ~ 0
RADDR0
Text Label 6650 3700 2    50   ~ 0
RADDR1
Text Label 6650 3800 2    50   ~ 0
RADDR2
Text Label 6650 3900 2    50   ~ 0
RADDR3
Text Label 6650 4000 2    50   ~ 0
RADDR4
Text Label 6650 4100 2    50   ~ 0
WADDR0
Text Label 6650 4200 2    50   ~ 0
WADDR1
Text Label 6650 4300 2    50   ~ 0
WADDR2
Text Label 6650 4400 2    50   ~ 0
WADDR3
Text Label 6650 4500 2    50   ~ 0
WADDR4
Text Label 6650 4600 2    50   ~ 0
COND0
Text Label 6650 4700 2    50   ~ 0
COND1
Text Label 6650 4800 2    50   ~ 0
COND2
Text Label 6650 4900 2    50   ~ 0
COND3
Text Label 6650 5000 2    50   ~ 0
COND4
Text Label 6650 5100 2    50   ~ 0
ACTION0
Text Label 6650 5200 2    50   ~ 0
ACTION1
Text Label 6650 5300 2    50   ~ 0
ACTION2
Text Label 6650 5400 2    50   ~ 0
ACTION3
Wire Wire Line
	6200 5800 6650 5800
Wire Wire Line
	6200 5900 6650 5900
Wire Wire Line
	6200 5700 6650 5700
Wire Wire Line
	6200 5600 6650 5600
Wire Wire Line
	6200 5500 6650 5500
Text Label 6650 6100 2    50   ~ 0
FPD[0..7]
Wire Wire Line
	5050 6100 4700 6100
Wire Wire Line
	5050 6000 4700 6000
Wire Wire Line
	5050 5900 4700 5900
Wire Wire Line
	5050 5800 4700 5800
Text Label 4650 3600 0    50   ~ 0
UPC0
Text Label 4650 3700 0    50   ~ 0
UPC1
Text Label 4650 3800 0    50   ~ 0
UPC2
Text Label 4650 3900 0    50   ~ 0
UPC3
Text Label 4650 4000 0    50   ~ 0
IDX0
Text Label 4650 4100 0    50   ~ 0
IDX1
Text Label 4650 4300 0    50   ~ 0
IN-RSVD
Text Label 4650 4200 0    50   ~ 0
~COND
Wire Wire Line
	4650 4300 5050 4300
Wire Wire Line
	4650 4200 5050 4200
Wire Wire Line
	4650 4100 5050 4100
Wire Wire Line
	4650 4000 5050 4000
Wire Wire Line
	4650 3900 5050 3900
Wire Wire Line
	4650 3800 5050 3800
Wire Wire Line
	4650 3700 5050 3700
Wire Wire Line
	4650 3600 5050 3600
Text Notes 6400 3250 2    100  ~ 20
Front Panel Buffers
Text Label 4750 5900 0    50   ~ 0
~FPµC0
Text Label 4750 6000 0    50   ~ 0
~FPµC1
Text Label 4750 6100 0    50   ~ 0
~FPµC2
Wire Bus Line
	6200 6100 6650 6100
Wire Wire Line
	6200 5400 6650 5400
Wire Wire Line
	6200 5300 6650 5300
Wire Wire Line
	6200 5200 6650 5200
Wire Wire Line
	6200 5100 6650 5100
Wire Wire Line
	6200 5000 6650 5000
Wire Wire Line
	6200 4900 6650 4900
Wire Wire Line
	6200 4800 6650 4800
Wire Wire Line
	6200 4700 6650 4700
Wire Wire Line
	6200 4600 6650 4600
Wire Wire Line
	6200 4500 6650 4500
Wire Wire Line
	6200 4400 6650 4400
Wire Wire Line
	6200 4300 6650 4300
Wire Wire Line
	6200 4200 6650 4200
Wire Wire Line
	6200 4100 6650 4100
Wire Wire Line
	6200 4000 6650 4000
Wire Wire Line
	6200 3900 6650 3900
Wire Wire Line
	6200 3800 6650 3800
Wire Wire Line
	6200 3700 6650 3700
Wire Wire Line
	6200 3600 6650 3600
Text Label 4250 5500 2    50   ~ 0
~MEM
Text Label 4250 5600 2    50   ~ 0
~IO
Text Label 4250 5700 2    50   ~ 0
~R
Text Label 4250 5800 2    50   ~ 0
~WEN
Text Label 4250 5900 2    50   ~ 0
~END
Text Notes 3750 3250 2    100  ~ 20
Control Store
Text Label 4250 3600 2    50   ~ 0
RADDR0
Text Label 4250 3700 2    50   ~ 0
RADDR1
Text Label 4250 3800 2    50   ~ 0
RADDR2
Text Label 4250 3900 2    50   ~ 0
RADDR3
Text Label 4250 4000 2    50   ~ 0
RADDR4
Text Label 4250 4100 2    50   ~ 0
WADDR0
Text Label 4250 4200 2    50   ~ 0
WADDR1
Text Label 4250 4300 2    50   ~ 0
WADDR2
Text Label 4250 4400 2    50   ~ 0
WADDR3
Text Label 4250 4500 2    50   ~ 0
WADDR4
Text Label 4250 4600 2    50   ~ 0
COND0
Text Label 4250 4700 2    50   ~ 0
COND1
Text Label 4250 4800 2    50   ~ 0
COND2
Text Label 4250 4900 2    50   ~ 0
COND3
Text Label 4250 5000 2    50   ~ 0
COND4
Text Label 4250 5100 2    50   ~ 0
ACTION0
Text Label 4250 5200 2    50   ~ 0
ACTION1
Text Label 4250 5300 2    50   ~ 0
ACTION2
Text Label 4250 5400 2    50   ~ 0
ACTION3
$Sheet
S 2650 3450 1150 2600
U 5E34FD9B
F0 "Microcode Store" 50
F1 "cpu_microcode_store.sch" 50
F2 "UPC0" I L 2650 3600 50 
F3 "UPC1" I L 2650 3700 50 
F4 "UPC2" I L 2650 3800 50 
F5 "UPC3" I L 2650 3900 50 
F6 "IDX0" I L 2650 4000 50 
F7 "IDX1" I L 2650 4100 50 
F8 "~COND" I L 2650 4200 50 
F9 "IN-RSVD" I L 2650 4300 50 
F10 "IR7" I L 2650 4400 50 
F11 "IR8" I L 2650 4500 50 
F12 "IR9" I L 2650 4600 50 
F13 "IR10" I L 2650 4700 50 
F14 "IR11" I L 2650 4800 50 
F15 "IR12" I L 2650 4900 50 
F16 "IR13" I L 2650 5000 50 
F17 "IR14" I L 2650 5100 50 
F18 "IR15" I L 2650 5200 50 
F19 "~IRQSµC" I L 2650 5300 50 
F20 "~RSTHOLD" I L 2650 5400 50 
F21 "RADDR0" O R 3800 3600 50 
F22 "RADDR1" O R 3800 3700 50 
F23 "RADDR2" O R 3800 3800 50 
F24 "RADDR3" O R 3800 3900 50 
F25 "RADDR4" O R 3800 4000 50 
F26 "WADDR0" O R 3800 4100 50 
F27 "WADDR1" O R 3800 4200 50 
F28 "WADDR2" O R 3800 4300 50 
F29 "WADDR3" O R 3800 4400 50 
F30 "WADDR4" O R 3800 4500 50 
F31 "COND0" O R 3800 4600 50 
F32 "COND1" O R 3800 4700 50 
F33 "COND2" O R 3800 4800 50 
F34 "COND3" O R 3800 4900 50 
F35 "COND4" O R 3800 5000 50 
F36 "ACTION0" O R 3800 5100 50 
F37 "ACTION1" O R 3800 5200 50 
F38 "ACTION2" O R 3800 5300 50 
F39 "ACTION3" O R 3800 5400 50 
F40 "~MEM" O R 3800 5500 50 
F41 "~IO" O R 3800 5600 50 
F42 "~R" O R 3800 5700 50 
F43 "~WEN" O R 3800 5800 50 
F44 "~END" O R 3800 5900 50 
F45 "CLK" I L 2650 5650 50 
F46 "~OE" I L 2650 5900 50 
$EndSheet
Wire Wire Line
	3800 5800 4250 5800
Wire Wire Line
	3800 5900 4250 5900
Wire Wire Line
	3800 5700 4250 5700
Wire Wire Line
	3800 5600 4250 5600
Text Label 2250 3600 0    50   ~ 0
UPC0
Text Label 2250 3700 0    50   ~ 0
UPC1
Text Label 2250 3800 0    50   ~ 0
UPC2
Text Label 2250 3900 0    50   ~ 0
UPC3
Text Label 2250 5300 0    50   ~ 0
~IRQSµC
Text Label 2250 5400 0    50   ~ 0
~RSTHOLD
Text Label 2250 4000 0    50   ~ 0
IDX0
Text Label 2250 4100 0    50   ~ 0
IDX1
Text Label 2250 4500 0    50   ~ 0
IR8
Text Label 2250 4600 0    50   ~ 0
IR9
Text Label 2250 4700 0    50   ~ 0
IR10
Text Label 2250 4800 0    50   ~ 0
IR11
Text Label 2250 4900 0    50   ~ 0
IR12
Text Label 2250 5000 0    50   ~ 0
IR13
Text Label 2250 5100 0    50   ~ 0
IR14
Text Label 2250 5200 0    50   ~ 0
IR15
Text Label 2250 4300 0    50   ~ 0
IN-RSVD
Text Label 2250 4200 0    50   ~ 0
~COND
Text Label 2250 4400 0    50   ~ 0
IR7
Wire Wire Line
	2250 3900 2650 3900
Wire Wire Line
	2250 3800 2650 3800
Wire Wire Line
	2250 3700 2650 3700
Wire Wire Line
	2250 3600 2650 3600
Wire Wire Line
	3800 3600 4250 3600
Wire Wire Line
	3800 3700 4250 3700
Wire Wire Line
	3800 3800 4250 3800
Wire Wire Line
	3800 3900 4250 3900
Wire Wire Line
	3800 4000 4250 4000
Text Label 2250 5900 0    50   ~ 0
~CSE
Wire Wire Line
	3800 4100 4250 4100
Wire Wire Line
	3800 4200 4250 4200
Wire Wire Line
	3800 4300 4250 4300
Wire Wire Line
	3800 4400 4250 4400
Wire Wire Line
	3800 4500 4250 4500
Wire Wire Line
	3800 4600 4250 4600
Wire Wire Line
	3800 4700 4250 4700
Wire Wire Line
	3800 4800 4250 4800
Wire Wire Line
	3800 4900 4250 4900
Wire Wire Line
	3800 5000 4250 5000
Wire Wire Line
	3800 5100 4250 5100
Wire Wire Line
	3800 5200 4250 5200
Wire Wire Line
	3800 5300 4250 5300
Wire Wire Line
	3800 5400 4250 5400
Wire Wire Line
	3800 5500 4250 5500
Text Notes 8200 3250 2    100  ~ 20
Bus Hold
$Comp
L alexios:74HC85 U45
U 2 1 5F4A7223
P 2950 7650
F 0 "U45" H 2950 8317 50  0000 C CNN
F 1 "74HC85" H 2950 8226 50  0000 C CNN
F 2 "alexios:SOIC-16" H 2950 7675 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74LS85" H 2950 7675 50  0001 C CNN
	2    2950 7650
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C?
U 1 1 5F4ADA60
P 2600 7350
AR Path="/5CC0D65F/5F4ADA60" Ref="C?"  Part="1" 
AR Path="/5F4ADA60" Ref="C?"  Part="1" 
AR Path="/5DEAC282/5F4ADA60" Ref="C38"  Part="1" 
F 0 "C38" H 2509 7396 50  0000 R CNN
F 1 "100nF" H 2509 7305 50  0000 R CNN
F 2 "alexios:C_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 2600 7350 50  0001 C CNN
F 3 "~" H 2600 7350 50  0001 C CNN
	1    2600 7350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0120
U 1 1 5F4ADA67
P 2600 7450
F 0 "#PWR0120" H 2600 7200 50  0001 C CNN
F 1 "GND" H 2605 7277 50  0000 C CNN
F 2 "" H 2600 7450 50  0001 C CNN
F 3 "" H 2600 7450 50  0001 C CNN
	1    2600 7450
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0116
U 1 1 5F4ADA6D
P 2600 7250
F 0 "#PWR0116" H 2600 7100 50  0001 C CNN
F 1 "+5V" H 2615 7423 50  0000 C CNN
F 2 "" H 2600 7250 50  0001 C CNN
F 3 "" H 2600 7250 50  0001 C CNN
	1    2600 7250
	1    0    0    -1  
$EndComp
Connection ~ 2600 7250
Connection ~ 2600 7450
Wire Wire Line
	1150 2100 1250 2100
$Comp
L Device:R_Small R?
U 1 1 5DB1C764
P 1650 3800
AR Path="/5DB1C764" Ref="R?"  Part="1" 
AR Path="/5CC0D65F/5DB1C764" Ref="R?"  Part="1" 
AR Path="/5D54E677/5DB1C764" Ref="R?"  Part="1" 
AR Path="/5D62E1DC/5DB1C764" Ref="R?"  Part="1" 
AR Path="/5DEAC282/5DB1C764" Ref="R26"  Part="1" 
AR Path="/5F67D4B5/5DB1C764" Ref="R?"  Part="1" 
AR Path="/5D46B22D/5DB1C764" Ref="R?"  Part="1" 
F 0 "R26" H 1709 3754 50  0000 L CNN
F 1 "4.7kΩ" H 1709 3845 50  0000 L CNN
F 2 "alexios:R_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 1650 3800 50  0001 C CNN
F 3 "~" H 1650 3800 50  0001 C CNN
	1    1650 3800
	1    0    0    1   
$EndComp
$Comp
L Device:R_Small R?
U 1 1 5DD09005
P 1800 1600
AR Path="/5DD09005" Ref="R?"  Part="1" 
AR Path="/5CC0D65F/5DD09005" Ref="R?"  Part="1" 
AR Path="/5D54E677/5DD09005" Ref="R?"  Part="1" 
AR Path="/5D62E1DC/5DD09005" Ref="R?"  Part="1" 
AR Path="/5DEAC282/5DD09005" Ref="R24"  Part="1" 
AR Path="/5F67D4B5/5DD09005" Ref="R?"  Part="1" 
AR Path="/5D46B22D/5DD09005" Ref="R?"  Part="1" 
F 0 "R24" H 1741 1554 50  0000 R CNN
F 1 "4.7kΩ" H 1741 1645 50  0000 R CNN
F 2 "alexios:R_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 1800 1600 50  0001 C CNN
F 3 "~" H 1800 1600 50  0001 C CNN
	1    1800 1600
	-1   0    0    1   
$EndComp
$Comp
L Device:R_Small R?
U 1 1 5DD0C349
P 1350 1600
AR Path="/5DD0C349" Ref="R?"  Part="1" 
AR Path="/5CC0D65F/5DD0C349" Ref="R?"  Part="1" 
AR Path="/5D54E677/5DD0C349" Ref="R?"  Part="1" 
AR Path="/5D62E1DC/5DD0C349" Ref="R?"  Part="1" 
AR Path="/5DEAC282/5DD0C349" Ref="R25"  Part="1" 
AR Path="/5F67D4B5/5DD0C349" Ref="R?"  Part="1" 
AR Path="/5D46B22D/5DD0C349" Ref="R?"  Part="1" 
F 0 "R25" H 1291 1554 50  0000 R CNN
F 1 "4.7kΩ" H 1291 1645 50  0000 R CNN
F 2 "alexios:R_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 1350 1600 50  0001 C CNN
F 3 "~" H 1350 1600 50  0001 C CNN
	1    1350 1600
	-1   0    0    1   
$EndComp
Wire Wire Line
	1350 1700 1350 2100
$Comp
L power:+5V #PWR0101
U 1 1 5DD4145D
P 1800 1500
F 0 "#PWR0101" H 1800 1350 50  0001 C CNN
F 1 "+5V" H 1650 1550 50  0000 C CNN
F 2 "" H 1800 1500 50  0001 C CNN
F 3 "" H 1800 1500 50  0001 C CNN
	1    1800 1500
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0102
U 1 1 5DD4162D
P 1350 1500
F 0 "#PWR0102" H 1350 1350 50  0001 C CNN
F 1 "+5V" H 1200 1550 50  0000 C CNN
F 2 "" H 1350 1500 50  0001 C CNN
F 3 "" H 1350 1500 50  0001 C CNN
	1    1350 1500
	1    0    0    -1  
$EndComp
Text HLabel 1150 5850 0    50   Input ~ 0
~RESET
Wire Wire Line
	1150 5850 1550 5850
Text Label 1500 5950 2    50   ~ 0
~HALT
Text Notes 2250 6800 0    50   ~ 0
The Microcode Store is enabled\nwhen ~RESET~ and ~HALT~\nare both deasserted.
Text HLabel 4700 6100 0    50   Input ~ 0
~FPµC2
Text HLabel 4700 6000 0    50   Input ~ 0
~FPµC1
Text HLabel 4700 5900 0    50   Input ~ 0
~FPµC0
Text Notes 7000 4950 0    50   ~ 0
Note: The DFP adds bus hold\nto all other control outputs.
Connection ~ 7650 5300
Connection ~ 7650 5700
Connection ~ 7650 5600
Connection ~ 7650 5400
$Comp
L alexios:SN74ACT1071 U?
U 2 1 5DA7AA39
P 8000 5800
AR Path="/5DEAC282/5DEAA264/5DA7AA39" Ref="U?"  Part="1" 
AR Path="/5DEAC282/5DA7AA39" Ref="U47"  Part="2" 
AR Path="/5DA7AA39" Ref="U?"  Part="1" 
F 0 "U47" H 7900 5750 50  0000 L CNN
F 1 "SN74ACT1071" H 7900 5650 50  0000 L CNN
F 2 "alexios:SOIC-14" H 7700 6150 50  0001 C CNN
F 3 "https://www.ti.com/lit/ds/symlink/sn74act1071.pdf" H 7700 6150 50  0001 C CNN
	2    8000 5800
	1    0    0    -1  
$EndComp
Wire Wire Line
	7650 5600 7650 5700
Wire Wire Line
	7650 5400 7650 5300
$Comp
L power:GND #PWR0110
U 1 1 5E28DA00
P 7650 5700
AR Path="/5DEAC282/5E28DA00" Ref="#PWR0110"  Part="1" 
AR Path="/5DEAC282/5DEAA264/5E28DA00" Ref="#PWR?"  Part="1" 
F 0 "#PWR0110" H 7650 5450 50  0001 C CNN
F 1 "GND" H 7655 5527 50  0000 C CNN
F 2 "" H 7650 5700 50  0001 C CNN
F 3 "" H 7650 5700 50  0001 C CNN
	1    7650 5700
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C?
U 1 1 5E28D9FA
P 7650 5500
AR Path="/5CC0D65F/5E28D9FA" Ref="C?"  Part="1" 
AR Path="/5E28D9FA" Ref="C?"  Part="1" 
AR Path="/5DEAC282/5E28D9FA" Ref="C33"  Part="1" 
AR Path="/5DEAC282/5DEAA264/5E28D9FA" Ref="C?"  Part="1" 
F 0 "C33" H 7558 5546 50  0000 R CNN
F 1 "100nF" H 7558 5455 50  0000 R CNN
F 2 "alexios:C_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 7650 5500 50  0001 C CNN
F 3 "~" H 7650 5500 50  0001 C CNN
	1    7650 5500
	1    0    0    -1  
$EndComp
$Comp
L alexios:74LVC1G00 U46
U 2 1 5DF346BA
P 1350 7250
F 0 "U46" H 1430 7196 50  0000 L CNN
F 1 "74LVC1G00" H 1430 7105 50  0000 L CNN
F 2 "alexios:SOT-23-5_HandSoldering" H 1000 7250 50  0001 C CNN
F 3 "https://assets.nexperia.com/documents/data-sheet/74LVC1G00.pdf" H 1000 7250 50  0001 C CNN
	2    1350 7250
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C?
U 1 1 5DF3702C
P 900 7350
AR Path="/5CC0D65F/5DF3702C" Ref="C?"  Part="1" 
AR Path="/5DF3702C" Ref="C?"  Part="1" 
AR Path="/5DEAC282/5DF3702C" Ref="C37"  Part="1" 
F 0 "C37" H 809 7396 50  0000 R CNN
F 1 "100nF" H 809 7305 50  0000 R CNN
F 2 "alexios:C_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 900 7350 50  0001 C CNN
F 3 "~" H 900 7350 50  0001 C CNN
	1    900  7350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0119
U 1 1 5DF37032
P 900 7450
F 0 "#PWR0119" H 900 7200 50  0001 C CNN
F 1 "GND" H 905 7277 50  0000 C CNN
F 2 "" H 900 7450 50  0001 C CNN
F 3 "" H 900 7450 50  0001 C CNN
	1    900  7450
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0115
U 1 1 5DF37038
P 900 7250
F 0 "#PWR0115" H 900 7100 50  0001 C CNN
F 1 "+5V" H 915 7423 50  0000 C CNN
F 2 "" H 900 7250 50  0001 C CNN
F 3 "" H 900 7250 50  0001 C CNN
	1    900  7250
	1    0    0    -1  
$EndComp
Connection ~ 900  7250
Connection ~ 900  7450
$Comp
L alexios:74LVC1G00 U46
U 1 1 5DD994A9
P 1850 5900
F 0 "U46" H 1600 5700 50  0000 C CNN
F 1 "74LVC1G00" H 1600 5600 50  0000 C CNN
F 2 "alexios:SOT-23-5_HandSoldering" H 1500 5900 50  0001 C CNN
F 3 "https://assets.nexperia.com/documents/data-sheet/74LVC1G00.pdf" H 1500 5900 50  0001 C CNN
	1    1850 5900
	1    0    0    -1  
$EndComp
Text Notes 1200 6750 0    50   ~ 0
~RESET~\n\n  0\n  X\n  1
Text Notes 1500 6750 0    50   ~ 0
~HALT~\n\n  X\n  0\n  1
Text Notes 1850 6750 0    50   ~ 0
~CSE~\n\n  1\n  1\n  0
Wire Notes Line width 12 style solid
	1150 6300 2100 6300
Wire Notes Line width 12 style solid
	2100 6300 2100 6800
Wire Notes Line width 12 style solid
	1150 6300 1150 6800
Wire Notes Line style solid
	1150 6460 2100 6460
Wire Notes Line
	1740 6300 1740 6800
Wire Notes Line width 12 style solid
	1150 6800 2100 6800
Text Label 4750 5800 0    50   ~ 0
~FPµA0
$Sheet
S 5050 3450 1150 2750
U 5D82AC00
F0 "Sheet5D82ABFF" 50
F1 "cpu_microcode_fp.sch" 50
F2 "RADDR0" I R 6200 3600 50 
F3 "RADDR1" I R 6200 3700 50 
F4 "RADDR2" I R 6200 3800 50 
F5 "RADDR3" I R 6200 3900 50 
F6 "RADDR4" I R 6200 4000 50 
F7 "WADDR0" I R 6200 4100 50 
F8 "WADDR1" I R 6200 4200 50 
F9 "WADDR2" I R 6200 4300 50 
F10 "WADDR3" I R 6200 4400 50 
F11 "WADDR4" I R 6200 4500 50 
F12 "COND0" I R 6200 4600 50 
F13 "COND1" I R 6200 4700 50 
F14 "COND2" I R 6200 4800 50 
F15 "COND3" I R 6200 4900 50 
F16 "COND4" I R 6200 5000 50 
F17 "ACTION0" I R 6200 5100 50 
F18 "ACTION1" I R 6200 5200 50 
F19 "ACTION2" I R 6200 5300 50 
F20 "ACTION3" I R 6200 5400 50 
F21 "~MEM" I R 6200 5500 50 
F22 "~IO" I R 6200 5600 50 
F23 "~R" I R 6200 5700 50 
F24 "~WEN" I R 6200 5800 50 
F25 "~END" I R 6200 5900 50 
F26 "FPD[0..7]" I R 6200 6100 50 
F27 "~FPµA0" I L 5050 5800 50 
F28 "~FPµC0" I L 5050 5900 50 
F29 "~FPµC1" I L 5050 6000 50 
F30 "~FPµC2" I L 5050 6100 50 
F31 "UPC0" I L 5050 3600 50 
F32 "UPC1" I L 5050 3700 50 
F33 "UPC2" I L 5050 3800 50 
F34 "UPC3" I L 5050 3900 50 
F35 "IDX0" I L 5050 4000 50 
F36 "IDX1" I L 5050 4100 50 
F37 "~COND" I L 5050 4200 50 
F38 "IN-RSVD" I L 5050 4300 50 
$EndSheet
Text Label 2250 5650 0    50   ~ 0
CLK2
Text HLabel 1150 4000 0    50   Input ~ 0
IDX0
Text HLabel 1150 4100 0    50   Input ~ 0
IDX1
Text HLabel 1150 4400 0    50   Input ~ 0
IR7
Text HLabel 1150 4500 0    50   Input ~ 0
IR8
Text HLabel 1150 4600 0    50   Input ~ 0
IR9
Text HLabel 1150 4700 0    50   Input ~ 0
IR10
Text HLabel 1150 4800 0    50   Input ~ 0
IR11
Text HLabel 1150 4900 0    50   Input ~ 0
IR12
Text HLabel 1150 5000 0    50   Input ~ 0
IR13
Text HLabel 1150 5100 0    50   Input ~ 0
IR14
Text HLabel 1150 5200 0    50   Input ~ 0
IR15
Wire Wire Line
	1150 4000 2650 4000
Wire Wire Line
	1150 4100 2650 4100
Wire Wire Line
	1150 4200 2650 4200
Wire Wire Line
	1150 4400 2650 4400
Wire Wire Line
	1150 4500 2650 4500
Wire Wire Line
	1150 4600 2650 4600
Wire Wire Line
	1150 4700 2650 4700
Wire Wire Line
	1150 4800 2650 4800
Wire Wire Line
	1150 4900 2650 4900
Wire Wire Line
	1150 5000 2650 5000
Wire Wire Line
	1150 5100 2650 5100
Wire Wire Line
	1150 5200 2650 5200
Wire Wire Line
	1150 5300 2650 5300
Text Label 1500 5850 2    50   ~ 0
~RESET
Entry Wire Line
	5000 1900 5100 2000
Entry Wire Line
	5000 2000 5100 2100
Entry Wire Line
	5000 2100 5100 2200
Entry Wire Line
	5000 2200 5100 2300
Text Label 5100 2750 1    50   ~ 0
UPC[0..3]
Entry Wire Line
	2150 3500 2250 3600
Entry Wire Line
	2150 3600 2250 3700
Entry Wire Line
	2150 3700 2250 3800
Entry Wire Line
	2150 3800 2250 3900
Entry Bus Bus
	5000 2950 5100 2850
Entry Bus Bus
	2250 2950 2150 3050
Text Label 2150 3400 1    50   ~ 0
UPC[0..3]
Wire Wire Line
	6400 2250 6850 2250
Connection ~ 6400 2250
Wire Wire Line
	6400 2300 6400 2250
Wire Wire Line
	6200 2150 6850 2150
Connection ~ 6200 2150
Wire Wire Line
	6200 2300 6200 2150
Wire Wire Line
	6400 2500 6400 2600
$Comp
L power:GND #PWR041
U 1 1 6047D252
P 6400 2600
F 0 "#PWR041" H 6400 2350 50  0001 C CNN
F 1 "GND" H 6400 2450 50  0000 C CNN
F 2 "" H 6400 2600 50  0001 C CNN
F 3 "" H 6400 2600 50  0001 C CNN
	1    6400 2600
	1    0    0    -1  
$EndComp
$Comp
L Device:R_Small R4
U 1 1 6047D24C
P 6400 2400
F 0 "R4" H 6350 2550 50  0000 R CNN
F 1 "4.7kΩ" H 6341 2445 50  0001 R CNN
F 2 "alexios:R_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 6400 2400 50  0001 C CNN
F 3 "~" H 6400 2400 50  0001 C CNN
	1    6400 2400
	1    0    0    1   
$EndComp
Wire Wire Line
	6200 2500 6200 2600
$Comp
L power:GND #PWR07
U 1 1 6046CC8E
P 6200 2600
F 0 "#PWR07" H 6200 2350 50  0001 C CNN
F 1 "GND" H 6200 2450 50  0000 C CNN
F 2 "" H 6200 2600 50  0001 C CNN
F 3 "" H 6200 2600 50  0001 C CNN
	1    6200 2600
	1    0    0    -1  
$EndComp
$Comp
L Device:R_Small R3
U 1 1 6046CC88
P 6200 2400
F 0 "R3" H 6150 2550 50  0000 R CNN
F 1 "4.7kΩ" H 6141 2445 50  0001 R CNN
F 2 "alexios:R_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 6200 2400 50  0001 C CNN
F 3 "~" H 6200 2400 50  0001 C CNN
	1    6200 2400
	1    0    0    1   
$EndComp
Wire Wire Line
	6000 2500 6000 2600
$Comp
L power:GND #PWR04
U 1 1 6044A2AC
P 6000 2600
F 0 "#PWR04" H 6000 2350 50  0001 C CNN
F 1 "GND" H 6000 2450 50  0000 C CNN
F 2 "" H 6000 2600 50  0001 C CNN
F 3 "" H 6000 2600 50  0001 C CNN
	1    6000 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	6000 2050 6850 2050
Connection ~ 6000 2050
Wire Wire Line
	6000 2300 6000 2050
$Comp
L Device:R_Small R1
U 1 1 603FB559
P 6000 2400
F 0 "R1" H 5950 2550 50  0000 R CNN
F 1 "4.7kΩ" H 5941 2445 50  0000 R CNN
F 2 "alexios:R_SMD_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 6000 2400 50  0001 C CNN
F 3 "~" H 6000 2400 50  0001 C CNN
	1    6000 2400
	1    0    0    1   
$EndComp
$Comp
L Jumper:SolderJumper_2_Open JP6
U 1 1 602E0B7C
P 6400 1350
F 0 "JP6" V 6400 1450 50  0000 L CNN
F 1 "SolderJumper_2_Bridged" H 6400 1464 50  0001 C CNN
F 2 "Jumper:SolderJumper-2_P1.3mm_Bridged_RoundedPad1.0x1.5mm" H 6400 1350 50  0001 C CNN
F 3 "~" H 6400 1350 50  0001 C CNN
	1    6400 1350
	0    1    1    0   
$EndComp
$Comp
L Jumper:SolderJumper_2_Bridged JP5
U 1 1 602E0974
P 6200 1350
F 0 "JP5" V 5900 1300 50  0000 L CNN
F 1 "SolderJumper_2_Bridged" H 6200 1464 50  0001 C CNN
F 2 "Jumper:SolderJumper-2_P1.3mm_Bridged_RoundedPad1.0x1.5mm" H 6200 1350 50  0001 C CNN
F 3 "~" H 6200 1350 50  0001 C CNN
	1    6200 1350
	0    1    1    0   
$EndComp
$Comp
L power:+5V #PWR01
U 1 1 602660E1
P 6000 1150
F 0 "#PWR01" H 6000 1000 50  0001 C CNN
F 1 "+5V" H 5850 1200 50  0000 C CNN
F 2 "" H 6000 1150 50  0001 C CNN
F 3 "" H 6000 1150 50  0001 C CNN
	1    6000 1150
	1    0    0    -1  
$EndComp
$Comp
L Jumper:SolderJumper_2_Open JP4
U 1 1 6025524A
P 6000 1350
F 0 "JP4" V 6000 1250 50  0000 R CNN
F 1 "SolderJumper_2_Bridged" H 6000 1464 50  0001 C CNN
F 2 "Jumper:SolderJumper-2_P1.3mm_Bridged_RoundedPad1.0x1.5mm" H 6000 1350 50  0001 C CNN
F 3 "~" H 6000 1350 50  0001 C CNN
	1    6000 1350
	0    1    1    0   
$EndComp
Wire Wire Line
	7850 2550 8300 2550
Text Notes 7750 1550 0    50   ~ 10
Set to 010 for Microcode v.7.
Text Notes 7750 2400 0    50   ~ 0
High = Fetch.\nLow = Execute.\nThis is for the benefit of the front panel only.
NoConn ~ 7850 2750
NoConn ~ 7850 2650
Wire Wire Line
	6750 2350 6750 2550
Wire Wire Line
	6850 2350 6750 2350
Text HLabel 8300 2550 2    50   Output ~ 0
FPFETCH
Text Label 8250 2550 2    50   ~ 0
FPFETCH
Wire Wire Line
	6700 2650 6850 2650
$Comp
L power:+5V #PWR0105
U 1 1 5FEA12FB
P 6700 2650
F 0 "#PWR0105" H 6700 2500 50  0001 C CNN
F 1 "+5V" V 6800 2650 50  0000 L CNN
F 2 "" H 6700 2650 50  0001 C CNN
F 3 "" H 6700 2650 50  0001 C CNN
	1    6700 2650
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0107
U 1 1 5FE9AF52
P 6750 2800
F 0 "#PWR0107" H 6750 2550 50  0001 C CNN
F 1 "GND" H 6755 2627 50  0000 C CNN
F 2 "" H 6750 2800 50  0001 C CNN
F 3 "" H 6750 2800 50  0001 C CNN
	1    6750 2800
	1    0    0    -1  
$EndComp
Wire Wire Line
	6750 2750 6750 2800
Connection ~ 6750 2750
Wire Wire Line
	6850 2750 6750 2750
Wire Wire Line
	6750 2550 6750 2750
Connection ~ 6750 2550
Wire Wire Line
	6850 2550 6750 2550
$Comp
L alexios:74HC85 U45
U 1 1 5FE49D00
P 7350 1950
F 0 "U45" H 7350 975 50  0000 C CNN
F 1 "74HC85" H 7350 884 50  0000 C CNN
F 2 "alexios:SOIC-16" H 7350 1975 50  0001 C CNN
F 3 "http://www.ti.com/lit/gpn/sn74LS85" H 7350 1975 50  0001 C CNN
	1    7350 1950
	1    0    0    -1  
$EndComp
Text Label 5200 1550 0    50   ~ 0
UPC0
Text Label 5200 1650 0    50   ~ 0
UPC1
Text Label 5200 1750 0    50   ~ 0
UPC2
Text Label 5200 1850 0    50   ~ 0
UPC3
Entry Wire Line
	5200 1550 5100 1650
Entry Wire Line
	5200 1650 5100 1750
Entry Wire Line
	5200 1750 5100 1850
Entry Wire Line
	5200 1850 5100 1950
Wire Wire Line
	5200 1550 6850 1550
Wire Wire Line
	5200 1850 6850 1850
Wire Wire Line
	5200 1750 6850 1750
Wire Wire Line
	5200 1650 6850 1650
Text Label 6800 1550 2    50   ~ 0
UPC0
Text Label 6800 1650 2    50   ~ 0
UPC1
Text Label 6800 1750 2    50   ~ 0
UPC2
Text Label 6800 1850 2    50   ~ 0
UPC3
Wire Wire Line
	6000 1500 6000 2050
Wire Wire Line
	6200 1500 6200 2150
Wire Wire Line
	6400 1500 6400 2250
Wire Wire Line
	6000 1150 6200 1150
Wire Wire Line
	6400 1150 6400 1200
Wire Wire Line
	6200 1200 6200 1150
Connection ~ 6200 1150
Wire Wire Line
	6200 1150 6400 1150
Wire Wire Line
	6000 1200 6000 1150
Connection ~ 6000 1150
Text Notes 6100 2350 0    98   ~ 20
*
Text Notes 7750 1750 0    98   ~ 20
*
Text Notes 7900 1750 0    50   ~ 0
Do not install this resistor by default.\nPopulate resistors for open jumpers only.
Text Notes 6750 1300 0    100  ~ 20
Fetch/Execute State Decoder
Text Notes 1400 3400 0    50   ~ 0
Remove\npull-up if\nIN-RSVD used.
Wire Wire Line
	1150 5400 2650 5400
Text HLabel 1150 4300 0    50   Input ~ 0
IN-RSVD
$Comp
L power:+5V #PWR055
U 1 1 6070CD65
P 1650 3650
F 0 "#PWR055" H 1650 3500 50  0001 C CNN
F 1 "+5V" H 1665 3823 50  0000 C CNN
F 2 "" H 1650 3650 50  0001 C CNN
F 3 "" H 1650 3650 50  0001 C CNN
	1    1650 3650
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 3650 1650 3700
Wire Wire Line
	1650 3900 1650 4300
Wire Wire Line
	1150 4300 1650 4300
Connection ~ 1650 4300
Wire Wire Line
	1650 4300 2650 4300
Text HLabel 1150 5650 0    50   Input ~ 0
CLK2
Text HLabel 4700 5800 0    50   Input ~ 0
~FPµA0
Entry Wire Line
	4550 3500 4650 3600
Entry Wire Line
	4550 3600 4650 3700
Entry Wire Line
	4550 3700 4650 3800
Entry Wire Line
	4550 3800 4650 3900
Entry Bus Bus
	4550 3050 4650 2950
Entry Bus Bus
	4550 3050 4450 2950
Wire Bus Line
	5000 2950 4650 2950
Wire Bus Line
	4450 2950 2250 2950
Wire Wire Line
	2000 5900 2650 5900
Wire Wire Line
	1150 5650 2650 5650
$Comp
L alexios:SN74ACT1071 U?
U 1 1 5E1680B3
P 7850 4250
AR Path="/5DEAC282/5DEAA264/5E1680B3" Ref="U?"  Part="1" 
AR Path="/5DEAC282/5E1680B3" Ref="U47"  Part="1" 
AR Path="/5E1680B3" Ref="U?"  Part="1" 
F 0 "U47" H 7800 5050 50  0000 L CNN
F 1 "SN74ACT1071" H 7600 4950 50  0000 L CNN
F 2 "alexios:SOIC-14" H 7550 4600 50  0001 C CNN
F 3 "https://www.ti.com/lit/ds/symlink/sn74act1071.pdf" H 7550 4600 50  0001 C CNN
	1    7850 4250
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0114
U 1 1 61FE2DF7
P 7200 4350
F 0 "#PWR0114" H 7200 4200 50  0001 C CNN
F 1 "+5V" V 7215 4478 50  0000 L CNN
F 2 "" H 7200 4350 50  0001 C CNN
F 3 "" H 7200 4350 50  0001 C CNN
	1    7200 4350
	0    -1   -1   0   
$EndComp
Wire Wire Line
	7200 4350 7300 4350
Wire Wire Line
	7300 4350 7300 4450
Wire Wire Line
	7300 4650 7350 4650
Connection ~ 7300 4350
Wire Wire Line
	7300 4350 7350 4350
Wire Wire Line
	7350 4450 7300 4450
Connection ~ 7300 4450
Wire Wire Line
	7300 4450 7300 4550
Wire Wire Line
	7350 4550 7300 4550
Connection ~ 7300 4550
Wire Wire Line
	7300 4550 7300 4650
Wire Wire Line
	1550 5950 1250 5950
Wire Wire Line
	1250 5950 1250 2100
Connection ~ 1250 2100
Wire Wire Line
	1250 2100 1350 2100
Wire Notes Line
	1350 3050 2000 3050
Wire Notes Line
	2000 3050 2000 3950
Wire Notes Line
	2000 3950 1350 3950
Wire Notes Line
	1350 3950 1350 3050
$Sheet
S 3400 1750 1150 900 
U 5F643CAF
F0 "Micro-Program Counter" 50
F1 "upc.sch" 50
F2 "UPC0" O R 4550 1900 50 
F3 "UPC1" O R 4550 2000 50 
F4 "UPC2" O R 4550 2100 50 
F5 "UPC3" O R 4550 2200 50 
F6 "~WAITING" I L 3400 2400 50 
F7 "CLK4" I L 3400 2000 50 
F8 "~ENDEXT" I L 3400 2300 50 
F9 "~END" I L 3400 2200 50 
F10 "~HALT" I L 3400 2100 50 
F11 "~RSTHOLD" I L 3400 1900 50 
$EndSheet
Wire Wire Line
	4550 1900 5000 1900
Wire Wire Line
	4550 2000 5000 2000
Wire Wire Line
	4550 2100 5000 2100
Wire Wire Line
	4550 2200 5000 2200
Text Label 2150 2400 0    50   ~ 0
~WAITING
Wire Wire Line
	2100 2400 3400 2400
Text HLabel 2100 2400 0    50   Input ~ 0
~WAITING
Wire Wire Line
	3400 1900 2150 1900
Text Label 2150 1900 0    50   ~ 0
~RSTHOLD
Text Label 2150 2000 0    50   ~ 0
CLK4
Text Label 2150 2100 0    50   ~ 0
~HALT
Text Label 2150 2200 0    50   ~ 0
~END
Text Label 2150 2300 0    50   ~ 0
~ENDEXT
Wire Wire Line
	1350 2100 3400 2100
Connection ~ 1350 2100
Wire Wire Line
	1150 2000 3400 2000
Wire Wire Line
	1150 2300 1800 2300
Wire Wire Line
	1800 1700 1800 2300
Connection ~ 1800 2300
Wire Wire Line
	1800 2300 3400 2300
Wire Wire Line
	2150 2200 3400 2200
Text Notes 700  1150 0    50   ~ 0
This is the inner sanctum of the control unit, reading, and decoding microprograms\ninto various fields.
Text Notes 700  850  0    100  ~ 20
The Microprogram Sequencer
Wire Bus Line
	4550 3050 4550 3800
Wire Bus Line
	2150 3050 2150 3800
Wire Bus Line
	9800 5100 9800 5450
Wire Bus Line
	9800 4400 9800 4850
Wire Bus Line
	9800 3700 9800 4150
Wire Bus Line
	9800 3000 9800 3450
Wire Bus Line
	5100 1650 5100 2850
$EndSCHEMATC
