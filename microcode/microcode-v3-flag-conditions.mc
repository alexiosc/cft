...
signal r_cs1          = ....................1110; // ALU: Read from the Constant Store (bank 1)
signal r_cs2          = ....................1111; // ALU: Read from the Constant Store (bank 2)
...
signal /incac         = ............1...........; // Step the A counter.
...
signal /incdr         = ......1.................; // Inc/Dec DR (for autoinc/autodec mode)
//signal RESERVED     = .....1..................; // 
...
start INT=X, RST=X, V=X, L=X, OP=XXXX, I=X, SKIP=X, INC=X;
      /end;
      /end;
      /end;
      /end;
...
start INT=X, RST=1, V=X, L=X, OP=XXXX, I=X, SKIP=X, INC=X;
...
start INT=X, RST=0, V=X, L=X, OP=XXXX, I=X, SKIP=X, INC=X;
      w_pc, r_cs1, /cli, /end;
      hold;
      hold;
      hold;
...
start INT=0, RST=1, V=X, L=X, OP=XXXX, I=X, SKIP=X, INC=X;
...
start INT=1, RST=1, V=X, L=X, OP=LOAD, I=0, SKIP=X, INC=X;
...
// Indirect mode.
start INT=1, RST=1, V=X, L=X, OP=LOAD, I=1, SKIP=X, INC=0;
...
start INT=1, RST=1, V=X, L=X, OP=LOAD, I=1, SKIP=X, INC=1;
...
// LITERAL MODE INSTRUCTION: agl used for I=0, direct mode for I=1.
start INT=1, RST=1, V=X, L=X, OP=STORE, I=0, SKIP=X, INC=X;
      _FETCH_IR;		// Fetch the instruction and operand.
      _MEMWRITE(agl, a);	// mem[agl] <- A
      _DESEL, /end;		// The fetch cycle begins with a memory read.

// Indirect mode.
start INT=1, RST=1, V=X, L=X, OP=STORE, I=1, SKIP=X, INC=0;
      _FETCH_LITERAL;		// Fetch the instruction and operand.
      _MEMWRITE(dr, a);		// mem[DR] <- A
      _DESEL, /end;

// FIXED: Autoindex mode
start INT=1, RST=1, V=X, L=X, OP=STORE, I=1, SKIP=X, INC=1;
      _FETCH_IR;                // Fetch cycle
      _MEMREAD(agl, dr);        // DR <- mem[agl]
      //_DESEL;			// Deselect all memory signals and wait.
...
start INT=1, RST=1, V=X, L=X, OP=ADD, I=1, SKIP=X, INC=0;
      _FETCH_LITERAL;		// Fetch the address and operand.
      //_DESEL;			// Prepare for the next memory read.
...
start INT=1, RST=1, V=X, L=X, OP=AND, I=0, SKIP=X, INC=X;
      _FETCH_IR;		// Fetch the instruction and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(agl, alu);       // B <- mem[DR].
      w_a, alu_and, /end;	// A <- A & B.
 
start INT=1, RST=1, V=X, L=X, OP=AND, I=1, SKIP=X, INC=0;
      _FETCH_LITERAL;		// Fetch the address and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(dr, alu);	// B <- mem[DR].
      w_a, alu_and, /end;	// A <- A & B.
...
// FIXED: Autoincrement
start INT=1, RST=1, V=X, L=X, OP=AND, I=1, SKIP=X, INC=1;
      _FETCH_LITERAL;		// Fetch the address and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(dr, alu);	// B <- mem[DR].
      w_a, alu_and;		// A <- A & B.
...
start INT=1, RST=1, V=X, L=X, OP=OR, I=0, SKIP=X, INC=X;
      _FETCH_IR;		// Fetch the instruction and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(agl, alu);       // B <- mem[DR].
      w_a, alu_or, /end;	// A <- A | B.
 
start INT=1, RST=1, V=X, L=X, OP=OR, I=1, SKIP=X, INC=0;
      _FETCH_LITERAL;		// Fetch the address and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(dr, alu);	// B <- mem[DR].
      w_a, alu_or, /end;	// A <- A | B.
...
// FIXED: Autoincrement
start INT=1, RST=1, V=X, L=X, OP=OR, I=1, SKIP=X, INC=1;
      _FETCH_LITERAL;		// Fetch the address and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(dr, alu);	// B <- mem[DR].
      w_a, alu_or;		// A <- A | B.
...
start INT=1, RST=1, V=X, L=X, OP=XOR, I=0, SKIP=X, INC=X;
      _FETCH_IR;		// Fetch the instruction and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(agl, alu);       // B <- mem[DR].
      w_a, alu_xor, /end;	// A <- A | B.
 
start INT=1, RST=1, V=X, L=X, OP=XOR, I=1, SKIP=X, INC=0;
      _FETCH_LITERAL;		// Fetch the address and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(dr, alu);	// B <- mem[DR].
      w_a, alu_xor, /end;	// A <- A | B.
...
// FIXED: Autoincrement
start INT=1, RST=1, V=X, L=X, OP=XOR, I=1, SKIP=X, INC=1;
      _FETCH_LITERAL;		// Fetch the address and operand.
      //_DESEL;			// Prepare for the next memory read.
      _MEMREAD(dr, alu);	// B <- mem[DR].
      w_a, alu_xor;		// A <- A | B.
...
start INT=1, RST=1, V=X, L=X, OP=JSR, I=0, SKIP=X, INC=X;
      _FETCH_IR;		// Fetch the instruction and operand. That's
				// all we need.
...
start INT=1, RST=1, V=X, L=X, OP=TRAP, I=0, SKIP=X, INC=X;
      _FETCH_IR;		// Fetch the instruction and operand. That's
				// all we need.
...
//      w_a, r_cs1, if6; \	// 4. If bit 7: do nothing here \
//      /cll, if5; \		// 5. If bit 6: do nothing here \
//      w_a, alu_not, if4; \	// 6. If bit 5: complement A    \
//      /incac, if3; \		// 7. If bit 4: increment A     \
//      /cpl, ifroll; \		// 8. If bit 3: complement L    \
//      w_a, alu_roll; \		// 9. If roll: well, roll.      \
//      /end			// (*)
//
...
// And withoug (because they're multiline)
#define OP1COMMON_SKIP1 \
      w_a, r_cs1, if6; \
      /cll, if5; \
      w_a, alu_not, if4; \
