...
_does_table_end:
	...
	.equ _does_table_size @_does_table_end-_does_table
...
.equ    TMPOUT  R &60		; Temporary (early development) tty out
...
//.reg    TIBP0   R &094
	...
	// The DOES> jump table pointer is the address of the first available slot in
// the table.
.reg	DTP	R &094
...
// The doDOES handler.
//
// This is the bastard child of doVAR and doCOL: RETV contains the PFA. It's
// pushed onto the stack.  RTTV contains the address of the DOES> code to
// execute, and this executed using doCOL.

_doDOES:
	LOAD RETV		; Get the PFA and push it onto the stack.
	PUSH SP
	
	LOAD IP			; Push the IP onto the return stack.
	PUSH RP
	LOAD RTTV		; Load the return address (PC+1 before TRAP, i.e. the DOES> code)
	STORE IP		; Save it as the new IP
	MACRO_NEXT()
...
// includes code to save and restore registers. Within, it calls the UISR
...
isr_done_tty0:
	...
	AND ISRR0
	SNZ
	JMP isr_done_kbd	; It wasn't IRQ1

	LI #0010		; IRQ1, disable
	OUT R &30		; Clear IRQ1 by strobing IEN1
	LI #0011		; IRQ1, enable
	OUT R &30
	...
	AND ISRR0
	SNZ
	JMP isr_done_kbd	; It was IRQ1, but not the keyboard.

	IN VDU_KBD		; Clear VDU KBIS interrupt flag, read KBD data.
	AND BYTELO
	STORE ISRR0
	...
	LOAD ISRR0
	AND BYTELO		; Return last byte OR flags
	OR VDUKBST		; Add extended/release state bits
	STORE ISRR0		; Used below to test for modifiers
	OR BIT15		; OR &8000 (new key seen!)
	STORE VDUKEY

	LI 0			; Clear state
	STORE VDUKBST
	JMP isrkbd_mods	; Done.

isrkbd_have_e0:
	LOR(VDUKBST, VDUKBST, &80)	; Set bit7 for extended codes
	JMP isr_done_kbd
	
isrkbd_have_f0:
	LOR (VDUKBST, VDUKBST, &100) ; Set bit8 for release events
	JMP isr_done_kbd
...
isrkbd_mods:
	LI 1
	STORE ISRR1		; ISRR1 = bit value
	LIA isr_moddata
	STORE IIR0
	RMOV(ISRR2, I IIR0)	; ISRR2 = count
	
isr_modloop:
	LOAD I IIR0
	XOR ISRR0		; match make code?
	SNZ
	JMP isr_modloop_make

	XOR PLUS256		; match break code? (&80)
	SNZ
	JMP isr_modloop_break

isr_modloop2:
	RSBL(ISRR1, ISRR1)	; next bit value
...
isr_modloop_make:
	ROR(VDUKBM, VDUKBM, ISRR1) ; Set a bit.
	JMP isr_modloop2
	
isr_modloop_break:
	LOAD ISRR1		; Clear a bit.
	NOT
	AND VDUKBM
	STORE VDUKBM
	JMP isr_modloop2
...
isr_moddata:
	...
	.word &59		; right shift
	.word &14		; left ctrl
	.word &80 &14		; right ctrl
	.word &11		; left alt
	.word &80 &11		; right alt
	.word &80 &1f		; left super
	.word &80 &27		; right super
