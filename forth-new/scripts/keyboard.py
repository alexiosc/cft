...
def dokeymap(n):
    ...
    for x in getheap(heap):
        ...
        if len(x) == 2:
            print "        .word &%02x &%02x00 ; %04x: %3s %3s" % \
                (x[0], x[1], addr, maybechar(x[0]), maybechar(x[1]))
        else:
            print "        .word     &%02x00 ; %04x:       %3s" % \
...
numkeys = len(heap)
print "_kbd_heapsz:"
print "        .word &%(numkeys)04x ; %(numkeys)d keys follow" % locals()
print "_kbd_heap:"
...
#print "\n_kbd_map_ctrl:\t; Ctrl+key keymap"
#dokeymap(2)

#print "\n_kbd_map_alt:\t; Alt+key keymap"
#dokeymap(3)
