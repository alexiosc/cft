...
panel_write(uint16_t addr, uint16_t dbus)
{
	...
	case IO_PANEL_LIGHTS:
		output_register = dbus;
		return 1;

// The tick counter was only implemented for Verilog reasons and is
// not meant for hardware implementations.
#if 0
	case IO_TICKS_HI:
	case IO_TICKS_LO:
		tickref = cpu.tick;
