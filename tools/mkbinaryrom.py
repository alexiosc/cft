            ...
            for l_in in xrange(2):
                for a in xrange(64):
                    for b in xrange(64):
                        ...
                        data = tt.make_vector(tt.outputs, l_out=l_out, x=x)
                        tt.put(addr, data)
        
                        if debug:
                            print '%s %05x %d %s (%d) %s %s (%d) = %s %s (%d)' % \
                                (self.opcodes[op],
                                 addr,
                                 l_in,
...
binaryROM = romtables.FunctionTable('op:2 l_in a:6 b:6', 'l_out x:6', singleROM=True)
